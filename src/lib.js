...
const files = {
  leafletjs: fs.readFileSync(
    require.resolve("leaflet/dist/leaflet.js"),
    "utf8",
  ),
  leafletcss: fs.readFileSync(
    require.resolve("leaflet/dist/leaflet.css"),
    "utf8",
  ),
  leafletpolylinedecorator: fs.readFileSync(
    require.resolve(
      "leaflet-polylinedecorator/dist/leaflet.polylineDecorator.js",
    ),
    "utf8",
  ),
  mapboxjs: fs.readFileSync(
    require.resolve("mapbox-gl/dist/mapbox-gl.js"),
    "utf8",
  ),
  mapboxcss: fs.readFileSync(
    require.resolve("mapbox-gl/dist/mapbox-gl.css"),
    "utf8",
  ),
  leafletmapboxjs: fs.readFileSync(
    require.resolve("mapbox-gl-leaflet/leaflet-mapbox-gl.js"),
    "utf8",
  ),
  markericonpng: new Buffer.from(
    fs.readFileSync(require.resolve("leaflet/dist/images/marker-icon.png")),
    "binary",
  ).toString("base64"),
};
...
class Browser {
  ...
  async getBrowser() {
    if (this.openingBrowser) {
      throw new Error(
        "osm-static-maps is not ready, please wait a few seconds",
      );
    }
  ...
  return new Promise((resolve, reject) => {
    let req = httpx.get(url, (res) => {
      if (res.statusCode !== 200) {
        reject(
          `Error ${res.statusCode} trying to get geojson file from ${url}`,
        );
      }
...
module.exports = function (options) {
  return new Promise(function (resolve, reject) {
    ...
    options.geojson =
      (options.geojson &&
        (typeof options.geojson === "string"
          ? options.geojson
          : JSON.stringify(options.geojson))) ||
      "";
    ...
    options.attribution =
      options.attribution || "osm-static-maps | © OpenStreetMap contributors";
    ...
    options.scale =
      (options.scale &&
        (typeof options.scale === "string"
          ? options.scale
          : JSON.stringify(options.scale))) ||
      false;
    options.markerIconOptions =
      (options.markerIconOptions &&
        (typeof options.markerIconOptions === "string"
          ? options.markerIconOptions
          : JSON.stringify(options.markerIconOptions))) ||
      false;
    options.style =
      (options.style &&
      ...
      false;
    options.timeout =
      typeof options.timeout == undefined ? 20000 : options.timeout;
    ...
    (async () => {
      if (options.geojsonfile) {
        if (options.geojson) {
          throw new Error(
            `Only one option allowed: 'geojsonfile' or 'geojson'`,
          );
        }
        if (
          options.geojsonfile.startsWith("http://") ||
          options.geojsonfile.startsWith("https://")
        ) {
          options.geojson = await httpGet(options.geojsonfile);
        } else {
          options.geojson = fs.readFileSync(
            options.geojsonfile == "-" ? process.stdin.fd : options.geojsonfile,
            "utf8",
          );
        }
      }
      ...
      try {
        ...
        if (options.imagemin) {
          ...
          (async () => {
            resolve(
              await imagemin.buffer(imageBinary, {
                plugins,
              }),
            );
          })();
